...
import (
	...
	user_event "github.com/cirrostratus-cloud/auth/event"
	muser "github.com/cirrostratus-cloud/auth/mocks/github.com/cirrostratus-cloud/auth/user"
	"github.com/cirrostratus-cloud/auth/mocks/github.com/cirrostratus-cloud/common/email"
	mevent "github.com/cirrostratus-cloud/auth/mocks/github.com/cirrostratus-cloud/common/event"
	"github.com/cirrostratus-cloud/auth/user"
	"github.com/cirrostratus-cloud/auth/util"
...
func TestRequestRecoverPasswordOk(t *testing.T) {
	...
	emailService.
		...
		Return(nil).
		Times(1)
	requestPasswordRecovery := user.NewRequestPasswordRecoveryService(userRepository, emailService, "cirrostratus-cloud@cloud.com", "https://example.com/reset", 3600, []byte("somekey"))
...
func TestRecoverPasswordInvalidToken(t *testing.T) {
	...
	eventBus := mevent.NewMockEventBus(t)
	validatePasswordService := user.NewValidatePasswordService(userRepository, true, true, true, true, 8)
	recoverPasswordService := user.NewRecoverPasswordService(userRepository, eventBus, validatePasswordService, []byte("somekey"))
...
func TestRecoverPasswordUserNotFound(t *testing.T) {
	...
	userRepository.
		...
		Times(1)
	eventBus := mevent.NewMockEventBus(t)
	token, err := util.GenerateTokenWithExpiration(time.Now().Add(time.Hour*24), map[string]interface{}{"user_id": ""}, []byte("somekey"))
	assert.Nil(err)
	validatePasswordService := user.NewValidatePasswordService(userRepository, true, true, true, true, 8)
	recoverPasswordService := user.NewRecoverPasswordService(userRepository, eventBus, validatePasswordService, []byte("somekey"))
	_, err = recoverPasswordService.RecoverPassword(user.RecoverPasswordRequest{
...
func TestNotifyEmailConfirmationOk(t *testing.T) {
	...
	emailService.
		...
		Return(nil).
		Times(1)
	notifyEmailConfirmationService := user.NewNotifyEmailConfirmationService(userRepository, emailService, eventBus, "cirrostratus-cloud@cloud.com", "https://example.com/confirm", []byte("somekey"), 3600)
...
func TestConfirmateEmailOk(t *testing.T) {
	...
	eventBus.
		...
		Return(nil).
		Times(1)
	token, err := util.GenerateTokenWithExpiration(time.Now().Add(time.Hour*24), map[string]interface{}{"user_id": "someid"}, []byte("somekey"))
	assert.Nil(err)
	confirmateEmailService := user.NewConfirmateEmailService(userRepository, eventBus, []byte("somekey"))
